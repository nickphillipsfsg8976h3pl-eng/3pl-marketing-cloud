SELECT cws.ContactKey,
    ...
    SELECT
        cws1.*,
        ROW_NUMBER() OVER(PARTITION BY cws1.ContactKey ORDER BY cws1.SubscriptionStartDate DESC) AS rn
    FROM
        ContactsWithSubscriptions_V1 cws1
    WHERE
    cws1.SubscriptionProduct LIKE '%Writing Legends%' AND
    cws1.SubscriptionRecordType IN ('Full','Extension') AND
    cws1.SubscriptionStatus = 'Active' AND
    cws1.HasOptedOutOfEmail = 0 AND
    cws1.ContactTerritory IN ('APAC','Africa') AND
    cws1.AccountBillingCountry IN ('Australia','South Africa','New Zealand') AND
    cws1.SubscriptionTerritory IN ('APAC','Africa') AND
