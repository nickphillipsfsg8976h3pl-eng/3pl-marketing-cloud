...
WHERE
    ...
    cws.SubscriptionEndDate    <= DATEADD(month, -1, GETDATE()) AND /*Changing to 1 month as per request*/
    ...
    AND NOT EXISTS
      (SELECT ContactKey
       FROM MX_APAC_LOST_RENEWALS_OLDER_THAN_3M_ES mx WHERE mx.ContactKey = cws.ContactKey )
