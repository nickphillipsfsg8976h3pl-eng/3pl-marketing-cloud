...
FROM
    ...
    LEFT JOIN ENT.User_Salesforce own ON lead.OwnerId = own.Id
WHERE
    lead.Status IN (
        'Closed - Lost',
        'Uncontactable – Nurture',
        'Unqualified'
    )
    AND lead.OwnerId != '00G2y000000YaqF'
    AND /*Parent Lead Queue ID*/ lead.Product_Interest__c LIKE '%Brightpath Progress Writing%'
    AND (
        lead.Reason_Lost__c NOT IN (
            'Fake',
            'Duplicate',
            'Parent Enquiry',
            'Already Subscribed',
        ...
        OR (
            lead.Reason_Lost__c = ''
            OR lead.Reason_Lost__c is null
        )
    )
    AND lead.IsConverted = 0
    AND lead.HasOptedOutOfEmail = 0
    AND lead.Email is NOT NULL
    AND lead.CountryCode IN ('AU', 'NZ')
    AND (
        (
            lead.Title NOT LIKE '%Parent%'
            AND lead.Title NOT LIKE '%Student%'
        )
        OR (
            lead.Title = ''
            OR lead.Title is null
        )
    )