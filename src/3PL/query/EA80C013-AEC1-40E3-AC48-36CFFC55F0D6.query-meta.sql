...
FROM (
    ...
    FROM (
        ...
        WHERE
            cws1.SubscriptionProduct LIKE '%Writing Legends%'
            AND cws1.SubscriptionRecordType = 'Trial'
            AND cws1.SubscriptionStatus = 'Active'
            AND cws1.HasOptedOutOfEmail = 0
            AND cws1.ContactTerritory = 'APAC'
            AND cws1.SubscriptionTerritory = 'APAC'
            AND cws1.AccountBillingCountryCode IN ('AU','NZ','ZA')
    ...
    WHERE
        ...
        AND ocr.Product__c LIKE '%Writing Legends%'
        AND rn = 1
) dedup
WHERE rn2 = 1
