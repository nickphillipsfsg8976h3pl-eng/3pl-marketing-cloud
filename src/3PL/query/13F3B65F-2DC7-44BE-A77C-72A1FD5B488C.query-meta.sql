...
FROM (
    ...
    FROM (
        ...
        WHERE
            cws1.SubscriptionProduct LIKE '%Reading Eggs%'
            AND cws1.SubscriptionRecordType = 'Trial'
            AND cws1.SubscriptionStatus = 'Active'
            AND cws1.HasOptedOutOfEmail = 0
            AND cws1.ContactTerritory = 'Americas' 
            AND cws1.SubscriptionTerritory = 'Americas' 
            AND cws1.AccountBillingCountryCode IN ('US','CA') 
    ...
    WHERE
        ...
        AND ocr.Product__c LIKE '%ReadingEggs%'
        AND rn = 1
) dedup
WHERE rn2 = 1
