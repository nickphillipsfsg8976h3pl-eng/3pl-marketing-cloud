...
LEFT JOIN  ENT.School__c_Salesforce sch
        ON sch.Account__c = a.Id
WHERE
    ...
    cws.SubscriptionEndDate    <= DATEADD(month, -9, GETDATE()) AND
    ...
    AND NOT EXISTS
      (SELECT ContactKey
       FROM MX_EME_LOST_RENEWALS_OLDER_THAN_9M_GT_18M_ES mx WHERE mx.ContactKey = cws.ContactKey )