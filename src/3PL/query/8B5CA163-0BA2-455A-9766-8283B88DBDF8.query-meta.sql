...
WHERE
    ...
    cws.SubscriptionRecordType IN ('Trial','Trial Extension') AND
    ...
    cws.AccountGlobalSchoolCategory = 'Primary' AND
    cws.ActiveFullSubscriptions NOT LIKE '%Reading Eggs%' AND
    cws.HasOptedOutOfEmail     = 0 AND
    cws.SubscriptionStartDate  <= DATEADD(DAY, -1, GETDATE()) AND
    cws.SubscriptionEndDate    <= DATEADD(DAY, -1, GETDATE()) AND
    ...
    cws.AccountBillingCountry  IN ('Australia','New Zealand','South Africa') AND
