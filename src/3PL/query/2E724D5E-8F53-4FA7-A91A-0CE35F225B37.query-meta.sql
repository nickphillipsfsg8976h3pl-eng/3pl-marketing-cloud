...
WHERE
    (((cws.SubscriptionProduct LIKE '%Mathseeds%' AND cws.ActiveFullSubscriptions NOT LIKE '%Reading%') AND
    ...
    c.Students_in_WL_Class__c IS NULL) OR
    ...
    cws.SubscriptionEndDate >= GETDATE() AND
    c.Is_Active__c = 'true' AND
    c.Students_in_WL_Class__c IS NULL)) AND
    cws.ContactEmail IS NOT NULL AND
    a.BillingCountryCode IN ('AU','NZ') AND
    c.RecordTypeID !='0120I000000pp8lQAA' AND
    a.RecordTypeID !='0120I0000019ZejQAE' AND
    
    not exists ( SELECT coo.AccountId
                 FROM Contacts_With_Open_Opportunities coo
                 WHERE coo.AccountId = cws.AccountId )
