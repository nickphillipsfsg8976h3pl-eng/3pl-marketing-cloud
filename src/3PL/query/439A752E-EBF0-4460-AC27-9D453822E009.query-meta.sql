SELECT cws.ContactKey,
    ...
    atm.UserId AS BDM_UserId,
    u.Name AS BDM_UserName,
    u.Email AS BDM_UserEmail,
    atm.Title As BDM_Title FROM (
    SELECT
        cws1.*,
...
LEFT JOIN
    ENT.AccountTeamMember_Salesforce atm
        ON atm.AccountId = cws.AccountId AND atm.TeamMemberRole = 'BDM'
LEFT JOIN
