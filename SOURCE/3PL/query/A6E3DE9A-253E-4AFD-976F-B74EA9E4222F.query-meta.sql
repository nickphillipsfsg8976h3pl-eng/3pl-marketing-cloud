...
FROM
    WA_BP_Staging sb  LEFT JOIN
    ENT.SBQQ__Subscription__c_Salesforce sub ON sb.SubscriptionId = sub.Id INNER JOIN
    ENT.Account_Salesforce ac
        ON  sub.Shipping_Account__c = ac.Id INNER JOIN
    ENT.AccountContactRelation_Salesforce rel
        ON  ac.Id = rel.AccountId LEFT JOIN
    ENT.Contact_Salesforce c
        ON  rel.ContactId = c.Id 
