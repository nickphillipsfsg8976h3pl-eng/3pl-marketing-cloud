...
WHERE 
    ...
    (cmp.Region = 'CANADA' OR cmp.Region = 'AMER')
    AND cws.CampaignId = '701Mp00000V66grIAB'
    AND cws.CreatedDate >= '2025-03-18'
    AND (cws.Title NOT IN ('Student', 'Parent', 'Other', 'Home schooling parent') OR cws.Title IS NULL)
    AND NOT EXISTS (
        SELECT 1 FROM [AMER_MX0067_WMD2025_J3] amer 
        WHERE amer.CampaignMemberId = cws.Id
    )
    AND NOT EXISTS (
        SELECT 1 FROM [APAC_MX0067_WMD2025_J3] apac 
        WHERE apac.CampaignMemberId = cws.Id
    )
    AND NOT EXISTS (
        SELECT 1 FROM [EMEA_MX0067_WMD2025_J3] emea 
        WHERE emea.CampaignMemberId = cws.Id
    )